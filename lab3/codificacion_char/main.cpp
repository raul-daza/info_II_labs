...
void encoding1(char* input, unsigned int n, char *output);
void decoding1(char* output, unsigned int n, char *deoutput);
void encoding2(char *input, unsigned int n, char *output);
void decoding2(char *output, unsigned int n, char *deoutput);
...
int main()
{
    ...
    cin >> n;
    fstream fileinfo;
    fileinfo.open("C:/Users/raulD/OneDrive/Documentos/QT/untitled/textoplano.txt", ios::in);
    if (fileinfo.is_open())
    {
        char ch;
        char text[10000];
        unsigned int pointerText = 0;
        while (fileinfo.get(ch))
        {
            text[pointerText] = ch;
            pointerText++;
        }
        ...
        {
            char output[10000];
            encoding1(text, n, output);
            fstream binfile;
            binfile.open("C:/Users/raulD/OneDrive/Escritorio/binFileEncoding1.bin", ios::out | ios::binary);
            binfile << output;
            cout << "________________________________" << endl;
            cout << output << endl;
            cout << "________________________________" << endl;
            char decoOutput[10000];
            decoding1(output, n, decoOutput);
            fstream decodefile;
            decodefile.open("C:/Users/raulD/OneDrive/Escritorio/binFileDecoding1.txt", ios::out);
            decodefile << decoOutput;
            cout << "________________________________" << endl;
            cout << decoOutput << endl;
            cout << "________________________________" << endl;
            decodefile.close();
            binfile.close();
            fileinfo.close();
        ...
        {
            char output[10000];
            encoding2(text, n, output);
            fstream binfile;
            binfile.open("C:/Users/raulD/OneDrive/Escritorio/binFileEncoding2.bin", ios::out | ios::binary);
            binfile << output;
            char decoOutput[10000];
            decoding2(output, n, decoOutput);
            fstream decodefile;
            decodefile.open("C:/Users/raulD/OneDrive/Escritorio/binFileDecoding2.txt", ios::out);
            decodefile << decoOutput;
            decodefile.close();
            binfile.close();
            fileinfo.close();
...
void encoding1(char* input, unsigned int n, char *output)
{
    ...
    unsigned int i = 0;
    unsigned int input_length = 0, pointerOutput = 0;
    ...
    while(!finished)
    {
        ...
        for(unsigned int i = jumps; i <= n; i+=jumps)
        {
            ...
            if(pointerBit2 > 7)
            {
                ...
                // si se alcanzó el final del texto a codificar termina
                if (pointerChar2 >= input_length)
                {
                    finished = true;
                    break;
                }
                output[pointerOutput] = character; // se agrega el caracter codificado a la informacion codificada
                pointerOutput++;
        ...
        if (pointerBit3 > 7)
        {
            ...
            if (pointerBit3 > 0)
            {
                ...
                if (pointerChar2 != pointerChar3)
                {
                    ...
                    output[pointerOutput] = character;
                    pointerOutput++;
    ...
    output[pointerOutput] = character; // agrega el ultimo caracter codificado en el bucle
    pointerOutput++;
    i = 0;
...
void decoding1(char* output, unsigned int n, char *deoutput)
{
    ...
    unsigned int ones = 0, zeros = 0; // cantidad de ceros y unos
    unsigned int output_length = 0, pointerDeoutput = 0;
    ...
    while(!finished)
    {
        ...
        for(unsigned int i = jumps; i <= n; i+=jumps)
        {
            ...
            if(pointerBit2 > 7)
            {
                pointerChar2++; // se mueve al siguiente caracter de la lista
                // si se alcanzó el final del texto a codificar termina
                if (pointerChar2 >= output_length)
                {
                    finished = true;
                    break;
                }
                deoutput[pointerDeoutput] = character; // se agresa el caracter codificado a la informacion codificada
                pointerDeoutput++;
        ...
        if (pointerBit3 > 7)
        {
            ...
            if (pointerBit3 > 0)
            {
                ...
                {
                    pointerChar2 = pointerChar3;
                    deoutput[pointerDeoutput] = character;
                    pointerDeoutput++;
    ...
    deoutput[pointerDeoutput] = character; // agrega el ultimo caracter codificado en el bucle
    pointerDeoutput++;
    int i = 0;
    // para mostrar en binario los caracteres codificados
    while (deoutput[i] != 0)
...
{
    cout << input << endl;
    char temp[10000]; // cadena temporal: almacena la cadena de entrada corrida a la derecha 1 posicion
    unsigned int pointerTemp = 0, pointerOutput = 0;
    ...
    while (input[i] != 0)
    {
        ...
        if (isOne)
        {
            temp[pointerTemp] = (character >> 1) | (1 << 7);
            pointerTemp++;
        }else
        {
            temp[pointerTemp] = character >> 1;
            pointerTemp++;
        }
    ...
    unsigned int input_length = 0, temp_length = 0;
    while(input[input_length] != 0)
    {
        input_length++;
    }
    ...
    while(pointerChar2 < temp_length)
    {
        if (pointerChar1 < input_length){
        ...
        if (pointerBit2 > 7)
        {
            output[pointerOutput] = character; // agrega el caracter modificado a la cadena de salida
            pointerOutput++;
            // si hay caracteres en la cadena temporal que no se modifica lo agrega a la cadena de salida
            for (unsigned int i = pointerChar2+1; i < ((pointerChar2 + pointerBit2/8) < temp_length ? (pointerChar2 + pointerBit2/8) : temp_length); i++)
            {
                output[pointerOutput] = temp[i];
                pointerOutput++;
            }
    ...
    if (input[input_length-1]%2 == 1 && input_length*8%n != 0)
    {
        output[pointerOutput] = (char)255;
        pointerOutput++;
    }else
    {
        output[pointerOutput] = (char)254;
        pointerOutput++;
    }
    i = 0;
    while (output[i] != 0)
...
void decoding2(char* output, unsigned int n, char *outputdeco)
{
    bool isFirstOne = (bool)(output[0] >> 7); // si el primer bit del caracter es 1
    unsigned int output_length = 0;
    unsigned int pointerTemp = 0, pointerOutputdeco = 0;
    char temp[10000];
    while(output[output_length] != 0)
    {
        output_length++;
    }
    pointerTemp = output_length - 2;
    int i = output_length-2;
    bool isOne = false;
    ...
    while (i != -1)
    {
        ...
        if (isOne)
        {
            temp[pointerTemp] = (char)((character << 1) | 1);
            pointerTemp--;
        }else
        {
            temp[pointerTemp] = (char)(character << 1);
            pointerTemp--;
        }
    ...
    unsigned short pointerBit1 = -1; // puntero1 para bitswap
    unsigned short pointerBit2 = (n+7)%8; // puntero2 para bitswap
    ...
    {
        outputdeco[pointerOutputdeco] = temp[i];
        pointerOutputdeco++;
    }
    while (temp[temp_length] != 0)
    {
    ...
    if (output[output_length-1] == (char)255)
    {
        outputdeco[pointerOutputdeco] = character | 1;
        pointerOutputdeco++;
    }else
    {
        outputdeco[pointerOutputdeco] = character;
